//Little Ottawa
//Bike Path, Wedding Horah, (Parliament Flame?) 
//
//Includes a booster button for when the bike path motors 
//stop and needs an extra voltage boost 

//Modifed from: 
//GPAC General Code
//By: Palmer Insull, palmeri@little-canada.ca
//Started: 7/6/2021
//Modified By: Nancy Pham, nancyp@little-canada.ca
...
unsigned long speedUpTime = 1500; //give momentary boost for 1 second  
...
//Profile 1 (Speed Profile) -- BIKE PATH #1
String Profile_1_Stages[5] = {"C"};
float Profile_1_Setpoints[5] = {0.5};
float Profile_1_Timings[5] = {0};

//Profile 2 (Speed Profile) -- BIKE PATH #2 
String Profile_2_Stages[5] = {"C"};
float Profile_2_Setpoints[5] = {0.45};
float Profile_2_Timings[5] = {0};
...
JOURNEY_CONFIGURATION Journey_1 {"Motor_1_Direction", no_servo, APIN1, 0, 0, 0, 12, 0, 0, 0, 0, Profile_1_Stages, Profile_1_Setpoints, Profile_1_Timings, true, true, 1}; //Bike Path #1
JOURNEY_CONFIGURATION Journey_2 {"Motor_1_Direction", no_servo, BPIN1, 0, 0, 0, 12, 0, 0, 0, 0, Profile_2_Stages, Profile_2_Setpoints, Profile_2_Timings, true, true, 1}; //Bike Path #2
JOURNEY_CONFIGURATION Journey_3 {"Servo", servo_1, DPIN1, 0, 0, 0, 12, 0, 0, 0, 0, Profile_3_Stages, Profile_3_Setpoints, Profile_3_Timings, true, true, 11}; //Wedding Horah
...
/*
  ...
  */
  ...
  //Button push will give a temporary voltage boost to BOTH magnorail bike paths! 
  pushButton = digitalRead(IN_1); 
  if (pushButton == 0){
    buttonFlag = 1; 
    previousTime = millis(); 
    Serial.println("ButtonPushed Time: " + String(previousTime)); 
  }
   ...
   // if (Journey.ACTIVE) {
      ...
      if (Journey.Device == "Servo") {
        servo_run(Journey);
          ...
          if (buttonFlag){
            boostTime = TIME - previousTime; 
            if (boostTime > speedUpTime){
              buttonFlag = 0; 
            }
            else{
              Journey.Speed = 0.99; 
              Serial.println("J: " + String(J) + " Speed: " + String(Journey.Speed));
            }
          }
